...
class HuffmanCoding:
    ...
    def create_coding_from_binary(self, data):
        frequency = self.make_frequency_dict(data)
        self.make_heap(frequency)
        self.merge_nodes()
        self.make_codes()

    def compress_from_binary(self, data):
        filename, file_extension = os.path.splitext(self.path)
    ...
    def decompress_from_binary(self):
        filename, file_extension = os.path.splitext(self.path)
        output_path = filename + "_encoded.p"
        with open(output_path, 'rb') as encoded_f:
            encoded_array = pickle.load(encoded_f)
            decoded_array = []
